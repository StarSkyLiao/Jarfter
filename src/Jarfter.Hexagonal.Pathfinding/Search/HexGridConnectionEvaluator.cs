...
internal static class HexGridConnectionEvaluator
{
    ...
    internal static bool TryGetBestConnection(
        ...
        HexagonalLayout layout,
        IReadOnlyDictionary<HexagonalCubePoint, HexGridSearchTypes.SparseNodeRecord> records,
        HexagonalCubePoint current,
        HexGridSearchTypes.SparseNodeRecord currentRecord,
        HexagonalCubePoint neighbor,
        HexagonalFootprint footprint,
        double clearanceApothemScale,
        IHexTraversalCostPolicy costPolicy,
        HexPathfindingStatisticsCollector? statisticsCollector,
        Dictionary<HexGridSearchTypes.LineOfSightCacheKey, HexGridSearchTypes.LineOfSightCacheEntry>? lineOfSightCache,
        bool useObstacleChunkAcceleration,
        ...
        out double cost)
    {
        ...
        {
            HexGridSearchTypes.SparseNodeRecord parentRecord = records[currentRecord.Parent];
            statisticsCollector?.AddLineOfSightQuery(true);
    ...
    private static bool TryGetTraversalCost(
        ...
        bool useObstacleChunkAcceleration)
    {
        HexGridSearchTypes.LineOfSightCacheKey key = new HexGridSearchTypes.LineOfSightCacheKey(start, end);

        if (lineOfSightCache is not null && lineOfSightCache.TryGetValue(key, out HexGridSearchTypes.LineOfSightCacheEntry cacheEntry))
        {
            statisticsCollector?.AddLineOfSightCacheHit();
        ...
        lineOfSightCache?.Add(key, new HexGridSearchTypes.LineOfSightCacheEntry(isTraversable, cost));
        return isTraversable;
    }
}
