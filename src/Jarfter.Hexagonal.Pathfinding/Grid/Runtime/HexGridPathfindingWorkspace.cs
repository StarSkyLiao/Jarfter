...
public sealed class HexGridPathfindingWorkspace
{
    ...
    /// <summary>
    /// 为指定中心稠密导航快照创建可复用的搜索工作区.
    /// </summary>
    /// <param name="snapshot">提供工作区容量与共享拓扑的中心稠密导航快照.</param>
    /// <exception cref="ArgumentNullException">当 <paramref name="snapshot"/> 为 <see langword="null"/> 时抛出.</exception>
    public HexGridPathfindingWorkspace(HexGridCentralNavigationSnapshot snapshot)
        : this(GetBake(snapshot))
    {
    }

    /// <summary>
    /// 使用调用方已验证的共享烘焙拓扑创建工作区.
    /// </summary>
    /// <param name="bake">定义工作区容量与稠密索引的不可变烘焙地图.</param>
    internal HexGridPathfindingWorkspace(HexGridCentralNavigationBake bake)
    {
    ...
    private static HexGridCentralNavigationBake GetBake(HexGridCentralNavigationSnapshot snapshot)
    {
        ArgumentNullException.ThrowIfNull(snapshot);
        return snapshot.Bake;
    }
}
