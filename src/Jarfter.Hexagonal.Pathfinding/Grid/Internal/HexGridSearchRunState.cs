...
internal sealed class HexGridSearchRunState
{
        ...
        bool usesStatelessLineOfSightCache)
    {
        ...
        LineOfSightCache = usesStatelessLineOfSightCache && HexGridSearchRuntime.ShouldEnableLineOfSightCache(mode, requestOptions)
            ? new Dictionary<HexGridSearch.LineOfSightCacheKey, HexGridSearch.LineOfSightCacheEntry>()
            : null;
    }
    ...
    /// 获取稀疏搜索后端跨阶段复用的直视缓存.
    /// </summary>
    internal Dictionary<HexGridSearch.LineOfSightCacheKey, HexGridSearch.LineOfSightCacheEntry>? LineOfSightCache { get; }
