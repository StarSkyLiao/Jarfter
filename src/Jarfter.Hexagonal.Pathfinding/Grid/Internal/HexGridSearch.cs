...
internal static class HexGridSearch
{
        ...
        HexGridSearchRunState? runState = null)
    {
        ...
        HexPathfindingStatisticsCollector? statisticsCollector = runState?.StatisticsCollector ?? (requestOptions?.CollectStatistics == true
            ...
            : null);
        Dictionary<LineOfSightCacheKey, LineOfSightCacheEntry>? lineOfSightCache = runState?.LineOfSightCache ?? (HexGridSearchRuntime.ShouldEnableLineOfSightCache(mode, requestOptions)
        ...
        Dictionary<HexagonalCubePoint, SparseNodeRecord> records = new Dictionary<HexagonalCubePoint, SparseNodeRecord>();
        ...
        records.Add(start, new SparseNodeRecord(0, default, false));
        openSet.Enqueue(
        ...
        {
            if (!records.TryGetValue(openNode.Point, out SparseNodeRecord currentRecord)
            ...
            for (int direction = 0; direction < 6; direction++)
            {
                ...
                if (records.TryGetValue(neighbor, out SparseNodeRecord existingRecord) && neighborCost >= existingRecord.Cost)
                {
                    continue;
                }

                records[neighbor] = new SparseNodeRecord(neighborCost, parent, true);
