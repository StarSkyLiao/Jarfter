...
public static partial class NumberExtension
{
    extension<T>(T value) where T : INumber<T>
    {
        ...
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public T Min(T min) => T.Max(value, min);
        ...
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public T Max(T max) => T.Min(value, max);
        ...
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public T Clamp01() => T.Clamp(value, T.Zero, T.One);
        ...
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public T Clamp(T min, T max) => T.Clamp(value, min, max);
        ...
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public T CircleClamp(T leftInclude, T rightExclude)
        {
            ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(rightExclude, leftInclude);

            T range = rightExclude - leftInclude;
            T offset = value - leftInclude;
            // 先归一化到 [0, range), 再平移回目标区间.
            offset %= range;
            if (offset < T.Zero) offset += range;
