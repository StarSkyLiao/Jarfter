...
public static partial class NumberExtension
{
    extension(int value)
    {
        ...
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public int Min(int min) => Math.Max(value, min);
        ...
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public int Max(int max) => Math.Min(value, max);
        ...
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public int Clamp01() => (uint)value <= 1u ? value : value < 0 ? 0 : 1;
        ...
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public int CircleClamp(int leftInclude, int rightExclude)
        {
            ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(rightExclude, leftInclude);

            int range = rightExclude - leftInclude;
            int offset = value - leftInclude;
            if (range > 0 && (uint)offset < (uint)range) return value;
            offset %= range;
