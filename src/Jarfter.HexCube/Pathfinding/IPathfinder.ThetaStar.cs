...
public partial interface IPathfinder
{
    ...
    /// <param name="heuristicWeight">启发函数权重. 必须为有限正数; 小于 1 时降低启发强度, 等于 1 时保持当前 Theta* 行为, 大于 1 时优先搜索速度而可能返回代价更高的路径.</param>
    /// <exception cref="ArgumentOutOfRangeException">当 <paramref name="heuristicWeight"/> 不是有限正数时抛出.</exception>
    public sealed class ThetaStar(IHeuristic heuristic, IThetaStarNavigationProvider navigationProvider, double heuristicWeight = 1) : IPathfinder
    {
